...
def test_process_batch(mocker):
    class MockInstanceBuilder:
        ...
        def parse(self):
            if self.raise_exception:
                raise XMLSyntaxError("message", 1, 0, 0)  # ty:ignore[invalid-argument-type, too-many-positional-arguments] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
            return self
    ...
    results = process_batch(instances, table_defs)  # ty:ignore[invalid-argument-type] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR

    for table in table_defs:
        pd.testing.assert_frame_equal(
            expected_dfs[table],
            results["dfs"][table].reset_index(drop=True),  # ty:ignore[invalid-argument-type] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
        )

    metadata = results["metadata"]  # ty:ignore[invalid-argument-type] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
