...
def test_columns_from_concepts(link_role, duration_column_names, instant_column_names):
    ...
    duration_columns = {field.name: field for field in duration_resource.schema_.fields}  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
    instant_columns = {field.name: field for field in instant_resource.schema_.fields}  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR

    assert duration_column_names == set(duration_columns.keys())
    assert instant_column_names == set(instant_columns.keys())

    # Verify data types
    assert duration_columns.get("duration_concept").type_ == "string"  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
    assert duration_columns.get("duration_concept_int").type_ == "integer"  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR

    assert instant_columns.get("instant_concept").type_ == "string"  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
    assert instant_columns.get("child_concept").type_ == "year"  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
    assert instant_columns.get("concept_bool").type_ == "boolean"  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
