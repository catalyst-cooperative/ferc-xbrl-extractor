...
class Period(BaseModel):
    ...
    def from_xml(cls, elem: Element) -> "Period":
        ...
        return cls(
            instant=False,
            start_date=elem.find(f"{{{XBRL_INSTANCE}}}startDate").text,  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
            end_date=elem.find(f"{{{XBRL_INSTANCE}}}endDate").text,  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
        )
...
class Axis(BaseModel):
    ...
    def from_xml(cls, elem: Element) -> "Axis":
        ...
        if elem.tag.endswith("typedMember"):
            dim = elem.getchildren()[0]  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
            return cls(
                name=elem.attrib["dimension"],
...
class Entity(BaseModel):
    ...
    def from_xml(cls, elem: Element) -> "Entity":
        ...
        return cls(
            identifier=elem.find(f"{{{XBRL_INSTANCE}}}identifier").text,  # ty:ignore[unresolved-attribute] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
...
class Context(BaseModel):
    ...
    def from_xml(cls, elem: Element) -> "Context":
        ...
        return cls(
            **{
                "c_id": elem.attrib["id"],
                "entity": Entity.from_xml(elem.find(f"{{{XBRL_INSTANCE}}}entity")),  # ty:ignore[invalid-argument-type] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
                "period": Period.from_xml(elem.find(f"{{{XBRL_INSTANCE}}}period")),  # ty:ignore[invalid-argument-type] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
    ...
    def as_primary_key(self, filing_name: str, axes: list[str]) -> dict[str, str]:
        ...
        return {
            ...
            **date_dict,
            **axes_dict,
        }  # ty:ignore[invalid-return-type] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
    ...
    def __init__(
        ...
        if "report_date" in duration_facts:
            self.report_date = datetime.date.fromisoformat(
                duration_facts["report_date"][0].value  # ty:ignore[invalid-argument-type] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
            )
        else:
            # FERC 714 workaround - though sometimes reports with different
            # publish dates have the same certifying official date.
            self.report_date = datetime.date.fromisoformat(
                duration_facts["certifying_official_date"][0].value  # ty:ignore[invalid-argument-type] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
            )
...
class InstanceBuilder:
    ...
    def parse(self, fact_prefix: str = "ferc") -> Instance:
        ...
        facts = root.findall(f"{fact_prefix}:*", root.nsmap)  # ty:ignore[invalid-argument-type] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
...
def get_instances(instance_path: Path | io.BytesIO) -> list[InstanceBuilder]:
    ...
    return [
        InstanceBuilder(str(instance), instance.name.rstrip(instance.suffix))  # ty:ignore[missing-argument] -- pre-existing gap, not introduced by adopting ty; tracked for a follow-up typing cleanup PR
